...
It often isn't. Studies of commercial citation systems find that up to 57% of attached citations are *post-rationalised* — the model generates the claim from memory, then finds a document that looks topically related and points at it. The link exists. The support does not. In science, [scite.ai](https://scite.ai) built a business on exactly this gap, sorting citations into *supporting*, *contrasting*, and *mentioning*. In intelligence and security analysis — where a wrongly-sourced claim can end up in a brief that reaches a decision-maker — nobody is selling the equivalent.
